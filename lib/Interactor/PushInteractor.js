...
var Connection = {
  init : function(hostname) {
    ...
    if (process.env.https_proxy || process.env.http_proxy || process.env.FORCE_HTTP) {
      this._type = 'http';
    }
    else {
      this._type = 'tcp';
  ...
  change : function(hostname) {
    this._hostname = hostname;
    if (this._type == 'http') return;

    this.tcpSocket.close();
    this.tcpSocket = this.axonSock.connect(cst.REMOTE_PORT_TCP, this._hostname);
  },
  send : function(data) {
    var self = this;

    if (this._type == 'tcp') {
      this.tcpSocket.send(JSON.stringify(data));
    }
    else {
      HttpRequest.post({
        url  : self._hostname,
...
var PushInteractor = module.exports = {
  changeUrl : function(url) {
    ...
    this._hostname = Url.parse(url).hostname;

    Connection.change(this._hostname);
  },
  start : function(p) {
    ...
    this._hostname = Url.parse(p.url).hostname;

    Connection.init(this._hostname);
  ...
  sendFile : function(packet) {
    ...
    fs.readFile(file, function(err, data) {
      ...
      // Heapdump only available for TCP connections
      if (Connection._type == 'tcp')
