...
module.exports = function(God) {
   ...
   * @return
   */
  God.heapdump = function(opts, cb) {
    if (opts.id) { // Perform a heapdump for the given app id
      if (!(opts.id in God.clusters_db))
        return cb(null, {err: null, data: 'Unknown app id: ' + opts.id});

      var proc = God.clusters_db[opts.id];
      if (proc.pm2_env.status !== cst.ONLINE_STATUS)
        return cb(null, {err: null, data: 'App id:' + opts.id + ' is not online'});

      proc.kill('SIGUSR2');

      var save_path = proc.pm2_env.heapdump_path;
      cb(null, {err: null, data: 'Heapdump saved: ' + save_path});
    }
    else { // No id specified, perform a heapdump for PM2 itself
      var save_path = p.join(cst.PM2_ROOT_PATH, 'heapdumps', 'pm2.heapsnapshot');

      heapdump.writeSnapshot(save_path, function(err, filename) {
        if (err)
          return cb(null, {err: err, data: null});
        cb(null, {err: null, data: 'Heapdump saved: ' + save_path});
      });
    }
  };
