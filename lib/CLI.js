...
/**
 * CLI method for performing a heapdump
 * @method heapdump
 * @param {object} id
 * @return
 */
CLI.heapdump = function(id) {
  Satan.executeRemote('heapdump', {id: id}, function(err, data) {
    if (data.err) {
      printError(cst.PREFIX_MSG_ERR + 'Heapdump failed');
      exitCli(cst.ERROR_EXIT);
    }
    else {
      printOut(cst.PREFIX_MSG + data.data);
      exitCli(cst.SUCCESS_EXIT);
    }
  });
};
